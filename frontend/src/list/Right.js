...
export default function Right(props) {
  ...
  useEffect(() => {
    if (data) {
      let productDataFilter = applyingFilter(data, props.filter, props.sortBy);
    ...
    <>
      <div className='float-r w-[75%] flex '>
        <ul className="flex-col w-[100%]">
          {productData.length > 0 && productData.map((item, ind) => (
            <li className='' id={item._id}>
              <div className='flex border-white border-2 border-r-0'>
                ...
                <div div className="float-r w-[70%] m-6"><Link to='/product' state={{ id: item._id }}>
                  <div className="desc text-white text-3xl m-2">{item.name}</div>
                  <div className="price text-white text-3xl m-2">₹{item.price}</div></Link>

...
export default function Cart() {
  ...
  const { data, isFetched, isError } = useCartDetail();
  ...
  useEffect(() => {
    ...
    let items = cartData.map((obj) => (
      <li className='text-white m-5' key={obj._id}>
        <div className="box m-5 p-5 flex border-2 rounded-lg shadow-lg bg-gray-800">
          <div className="img flex-shrink-0">
            <img src={obj.Images.i1} width={150} height={150} alt="" className="rounded-lg" />
          </div>
          <div className="right ml-10 space-y-5 flex-grow">
            <div className="text-2xl font-bold text-white">{obj.name}</div>
  ...
  return (
    <div className="p-5">
      ...
      {isError && (
        <div className='text-center text-red-500'>Failed to load cart items. Please login.</div>
      )}
      {!isFetched && (
